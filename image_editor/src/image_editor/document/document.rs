...
use super::{rendering_strategy, RenderingStrategy};
...
impl<T: RenderingStrategy> Document<T> {
    pub fn new(config: DocumentCreationInfo, framework: &Framework, rendering_strategy: T) -> Self {
    ...
    pub(crate) fn render(&mut self) {
        ...
        for layer_node in self.tree_root.0.iter() {
            match layer_node {
                LayerTree::SingleLayer(index) => {
                    ...
                    self.rendering_strategy.render_layer(layer);
                }
        ...
        self.rendering_strategy.end();
    }

    pub(crate) fn update_gpu_data(&self, framework: &Framework) {
        let final_render = self.rendering_strategy.get_result();
        self.final_texture.write_data(&final_render, framework);
    }
    ...
    pub fn image_bytes(&mut self) -> &ImageBuffer<Rgba<u8>, Vec<u8>> {
        self.rendering_strategy.get_result()
    }
